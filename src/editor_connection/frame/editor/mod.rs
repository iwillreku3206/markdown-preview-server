...
impl Frame for EditorFrame {
    fn to_string(&self) -> String {
        match self {
			...
			EditorFrame::SetCursorPositon(line, column) => format!("set_cursor_position {} {}", line, column),
    ...
    fn to_vec(&self) -> Vec<u8> {
        match self {
			...
			EditorFrame::SetCursorPositon(line, column) => {
				...
				vec.extend_from_slice(column.to_string().as_bytes());
				vec
			},
